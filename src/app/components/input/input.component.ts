import { NoteI, CheckboxI, ImageI } from '../../interfaces/notes';
...
export class InputComponent implements OnInit {
  ...
  // For text formatting - save selection before opening color picker
  ...
  pasteEvent(event: ClipboardEvent) {
    ...
    // Handle text paste (original functionality)
  ...
  handleImagePaste(blob: Blob) {
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      ...
      const img = new Image()
      img.onload = () => {
        this.addImage(base64, img.width, img.height)
  ...
  addImage(imageData: string, width?: number, height?: number) {
    if (!this.images) {
      this.images = []
    }
    this.images.push({
  ...
  innerData(note: NoteI) {
    ...
    if (note.images) this.images = note.images
  ...
  colorMenu = {
    bgColor: (data: bgColors) => {
      ...
      // For light theme, keep the border visible instead of matching background
      if (data) {
        this.noteMain.nativeElement.style.borderColor = data
      } else {
        // Reset to default border when removing background color
        this.noteMain.nativeElement.style.borderColor = '#e0e0e0'
      }
    },
  ...
  formatText(command: string) {
    // Restore selection first in case it was lost
    this.restoreSelection()
    document.execCommand(command, false)
    this.noteBody?.nativeElement.focus()
  }

  // Called when clicking on text color button - save selection before tooltip opens
  openTextColorMenu(button: HTMLDivElement, tooltipEl: HTMLDivElement) {
    this.saveSelection()
  ...
  applyTextColor(color: string) {
    ...
    // Focus back on the note body
    this.noteBody?.nativeElement.focus()

    // Close the tooltip
    const tooltip = document.querySelector('[data-tooltip="true"][data-is-tooltip-open="true"]')
    if (tooltip) {
      this.Shared.closeTooltip(tooltip as HTMLDivElement)
    }
  }
